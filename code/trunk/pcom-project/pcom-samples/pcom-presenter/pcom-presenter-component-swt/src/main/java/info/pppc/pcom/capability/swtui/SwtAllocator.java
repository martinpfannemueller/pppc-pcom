 ...
 * @author Mac
 */
public class SwtAllocator implements IAllocator {


	/**
	 * The name of the allocator as shown in user interfaces.
	 */
	private static final String CAPABILITY_NAME = "3PC SWTUI";
	 ...
	 */
	public SwtAllocator(Application application) {
		if (application == null) 
	...
	public IResourceSetup[] deriveSetups(IResourceDemandReader demand) {
		...
		ITypeDemandReader iface = demand.getInterface(ISwtAccessor.class.getName());
		...
		rpro.createInterface(ISwtAccessor.class.getName());
		ITypeProvisionWriter ipro = rpro.getInterface(ISwtAccessor.class.getName());
	...
	public boolean startResource(IResourceContext context) {
		if (context.getAccessor() == null) {
			context.setAccessor(new SwtAccessor(application));	
		}
		return true;
	}
	...
	public void stopResource(IResourceContext context) { 
		SwtAccessor accessor = (SwtAccessor)context.getAccessor();
		if (accessor != null) {
			accessor.dispose();	
		}
	}
