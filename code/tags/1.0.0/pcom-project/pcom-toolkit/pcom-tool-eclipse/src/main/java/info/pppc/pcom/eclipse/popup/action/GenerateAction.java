...
import info.pppc.pcom.eclipse.generator.Generator;
...
/**
 * The generate action is used to generate a new component template
 * from a specified component descriptor that is contained in some
 * file. The action first tries to parse the document and if this
 * succeeds, it generates the desired files desribed in the descriptor.
 * 
 * @author Mac
 */
	...
	public void run(IAction action) {
		...
		try {
				...
				public void run(final IProgressMonitor monitor) throws InvocationTargetException, InterruptedException {
					try {
						try {
							...
							// parse document
							LocatorDocument d = parser.parse(selectedFile.getContents());
							Generator generator = new Generator();
							generator.init(project, d, monitor);
							generator.generate();
							monitor.worked(1);
			...
			if (throwable instanceof JavaModelException) {
				...
				ErrorDialog.openError(shell, "3PC Pcom Tools", "Generate component failed.", exception.getStatus());
			} else {
				Plugin plugin = Plugin.getDefault();
				plugin.error("Generate action failed.", throwable);
				...
				ErrorDialog.openError(shell, "3PC Pcom Tools", "Generate component failed.", status);				
			}
			...
			ErrorDialog.openError(shell, "3PC Pcom Tools", "Generation status unknown.", status);			
		}
	}
