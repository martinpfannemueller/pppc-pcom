...
/**
 * The resource binding represents a resource demand within the gc assembler.
 * It can be used by resources or instances.
 * 
 * @author Mac
 */
public class ResourceBinding extends AbstractBinding {
	...
	/**
	 * Creates a new resource binding using the specified application,
	 ...
	 */
	public ResourceBinding(Application application, AbstractItem parent, SystemID systemID,  Contract demand, Pointer pointer) {
		super(application, parent, systemID, demand);
		if (pointer != null) {
			this.pointer = new Pointer(pointer, false, demand.getName());	
		}
		...
		if (configured) return;
			...
			if (template < templates.size()) {
				...
				if (state != null && state.getSystemID().equals(sysID) && state.getCreatorID().equals(creID)) {
					element = new Resource(application, this, sysID, creID, temps, pointer);	
				} else {
					element = new Resource(application, this, sysID, creID, temps, null);
				}
	...
	/**
	 * Externalizes the resource binding into an assembly
